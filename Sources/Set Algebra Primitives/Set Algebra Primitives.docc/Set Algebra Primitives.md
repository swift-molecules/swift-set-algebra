...
`Membership` is the minimal membership core (`contains` + `count`). The set *algebra* is a third orthogonal concern, composed over the core and the iteration concern (``Iterable``) rather than baked into either:

- **Relational predicates** (`isDisjoint`, `isSubset`, `isSuperset`, `isStrictSubset`, `isStrictSuperset`, `isEqual`) attach `where Self: Membership & Iterable`. They enumerate via the iteration concern and probe via the membership core, so they work across *any* two conformers with the same element — even different set types.
...
Composed `where Self: Membership & Iterable` — see ``Membership``.
